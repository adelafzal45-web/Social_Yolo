...
export class RetrieverService {
   ...
   * When `category` is specified, filters/prioritizes posts belonging to that category.
   */
  async retrieveStyleContext(
    ...
    // Helper to check if a row matches the requested category
    const matchesCategory = (row: PostEmbedding): boolean => {
      ...
      return (
        rowCat === targetCategory ||
        rowCat.includes(targetCategory) ||
        targetCategory.includes(rowCat)
      );
    };

    if (userId) {
      const userScored = scored.filter(
        ({ row }) =>
          row.source === 'user' &&
      ...
      // Category-scoped user references first
      const categoryUserMatches = targetCategory
        ? userScored.filter(({ row }) => matchesCategory(row))
        : [];
      const chosenUser =
        categoryUserMatches.length > 0 ? categoryUserMatches : userScored;
    ...
    const sampleScored = scored.filter(({ row }) => row.source === 'sample');
    const categorySampleMatches = targetCategory
      ? sampleScored.filter(({ row }) => matchesCategory(row))
      : [];

    // Pick from category-specific samples, fallback to general if fewer than TOP_SAMPLE
    const chosenSamples = [
      ...categorySampleMatches.sort((a, b) => b.similarity - a.similarity),
    ];
    if (chosenSamples.length < TOP_SAMPLE) {
      const alreadyIds = new Set(chosenSamples.map((s) => s.row.id));
      const remaining = sampleScored
        .filter((s) => !alreadyIds.has(s.row.id))
        .sort((a, b) => b.similarity - a.similarity);
      chosenSamples.push(
        ...remaining.slice(0, TOP_SAMPLE - chosenSamples.length),
      );
    }
